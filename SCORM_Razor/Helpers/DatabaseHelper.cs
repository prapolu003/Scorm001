    ...
    public class DatabaseHelper
    {
        ...
        public bool isCourseInUserStudyArea(int SCORM_Course_id, string UserID)
        {
            ...
            try
            {
                ...
                {
                    var row = _db.User_Module.Where(ix => ix.UserID == UserID && ix.SCORM_Course.id == SCORM_Course_id).OrderByDescending(iy => iy.dtDateAdded).FirstOrDefault();
                    if (row != null)
                    {
                        b = true;
                    }
        ...
        public void AddCourseToUsersStudyArea(int SCORM_Course_id, string UserID)
        {
            try
            {
                using (var _db = getContext())
                {
                    ...
                    var SCORM_Course = _db.SCORM_Course.Where(ix => ix.id == SCORM_Course_id).FirstOrDefault();
                    user_module.SCORM_Course = SCORM_Course;
                    user_module.UserID = UserID;
                    _db.User_Module.Add(user_module);
                    _db.SaveChanges();
                }
            }
            catch (Exception ex)
        ...
        public void UpdateUserModuleDateLastOpened(string UserID, int SCORM_Course_id)
        {
            try
            {
                using (var _db = getContext())
                {
                    var user_module = _db.User_Module.Where(ix => ix.SCORM_Course.id == SCORM_Course_id && ix.UserID == UserID).OrderByDescending(iy => iy.dtDateAdded).FirstOrDefault();
        ...
        public void UpdateCore(int core_id, string entry, string lesson_status, string total_time)
        {
            try
            {
                using (var _db = getContext())
                {
                    var core = _db.cmi_core.Where(ix => ix.core_id == core_id).FirstOrDefault();
                    core.entry = entry;
        ...
        public void UpdateSession(int id, DateTime enddatetime)
        {
            try
            {
                using (var _db = getContext())
                {
                    var session = _db.session.Where(ix => ix.id == id).FirstOrDefault();
        ...
        public List<User_Module> SelectUserModule(string UserID)
        {
            using (var _db = getContext())
            {
                var rows = _db.User_Module.Where(ix => ix.UserID == UserID)
        ...
        public void InsertScormCourse(string titleFromManifest, string title, string pathToIndex, string pathToManifest, string pathToFolder, string SCORM_Version, DateTime dtUploaded, string UserID)
        {
            try
            {
                using (var _db = getContext())
                {
                    ...
                    _db.SCORM_Course.Add(SCORM_course);
                    _db.SaveChanges();
                }
            }
            catch (Exception ex)
        ...
        public int GetCmiDataID(string user_id, string sco_identifier, string sco_courses_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var cmi_data = _db.cmi_data.Where(ix => ix.SCORM_courses_id == iSCO_Course_id && ix.user_id == user_id).OrderByDescending(iy => iy.id).FirstOrDefault();
                if (cmi_data != null)
                {
        ...
        public cmi_core GetCMICoreById(int core_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    core = _db.cmi_core.Find(core_id);
                    }
            }
            catch (Exception ex)
            {
        ...
        public cmi_data GetCMIDataByID(int data_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    data = _db.cmi_data.Find(data_id);
                    }
            }
            catch (Exception ex)
            {
        ...
        public int getCountInteractionsByCoreID(int core_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_interactions.Select(ix => ix.core_id == core_id).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public cmi_interactions GetInteractionsByInteractionsID(int interactions_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    interactions = _db.cmi_interactions.Where(ix => ix.id == interactions_id).FirstOrDefault();
                    }
            }
            catch (Exception ex)
            {
        ...
        public string GetInteractionType(int cmi_interactions_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    interactions = _db.cmi_interactions.Where(ix => ix.id == cmi_interactions_id).FirstOrDefault();
        ...
        public int getCountObjectivesByCoreID(int core_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_objectives.Select(ix => ix.core_id == core_id).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public int getCountCommentsFromLearnerByCoreID(int core_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_comment_from_learner.Select(ix => ix.core_id == core_id).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public int GetCountInteractionsCorrectResponsesByInteractionsID(int interactions_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_interactions_correct_responses.Select(ix => ix.interactions_id == interactions_id).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public int GetCountInteractionsObjectivesByInteractionsID(int interactions_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_interactions_objectives.Select(ix => ix.interactions_id == interactions_id).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public int getCountCommentsFromLMS(int SCORM_Course_id, string SCO_identifier)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    iCount = _db.cmi_comment_from_lms.Select(ix => ix.SCORM_Course_id == SCORM_Course_id && ix.SCO_identifier == SCO_identifier).Count();
                    }
            }
            catch (Exception ex)
            {
        ...
        public List<SCORMCourse> getCourseList()
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var list = _db.SCORM_Course.Select(i => new { i.id, i.pathToIndex, i.pathToFolder, i.title_from_manifest, i.DateUploaded }).OrderBy(ix => ix.title_from_manifest).ToList();
        ...
        public List<SCORM_Course_fromSP> getCourseListWithUserIndicator(string UserId)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    listCourses = _db.SCORM_Course_FromSP
        ...
        public SCORMCourse getSCORMCourse(int SCORM_Course_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var j = _db.SCORM_Course.Select(i => new { i.id, i.pathToIndex, i.pathToFolder, i.title_from_manifest, i.DateUploaded }).Where(ix => ix.id == SCORM_Course_id).FirstOrDefault();
                    if (j != null)
                    {
            ...
            public int GetCoreTrackingID(int iSCORM_Course_ID, string UserId)
        {
            ...
            try
            {
                using (var _db = getContext())
                { 
                    var cmi_core = _db.cmi_core.FromSql($"EXECUTE dbo.Sel_CoreTrackingID {iSCORM_Course_ID}, {UserId}").ToList().FirstOrDefault();
                    //var cmi_core = _db.cmi_core.FromSqlRaw($"select * From dbo.cmi_core where student_id = @s", parameterS).ToList();
                    //var cmi = _db.cmi_core.FromSqlRaw($"Select * from cmi_core where student_id = @s", parameterS).ToList();
        ...
        public int GetSessionID(int iSCORM_Course_ID, string UserId, string sessionid, int iCore_id, DateTime dtStartTime)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var session = _db.session.FromSql($"dbo.Sel_SessionID {iSCORM_Course_ID},  null, {UserId}, {sessionid}, {iCore_id}, {dtStartTime}").ToList().FirstOrDefault();
        ...
        public int GetInteractionsID(int cmi_core_id, int n, bool bNew)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var cmi_interactions = _db.cmi_interactions.Where(ix => ix.core_id == cmi_core_id && ix.n == n).FirstOrDefault();
            ...
            else if (bNew)
            {
                try
                {
                    using (var _db = getContext())
                    {
                        ...
                        //if (max_n == n)
                        //{
                        DbCommand cmd1 = _db.Database.GetDbConnection().CreateCommand();
        ...
        public int GetCommentsFromLearnerID(int core_id, int n, bool bNew)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var cmi_comment_from_learner = _db.cmi_comment_from_learner.Where(ix => ix.core_id == core_id).FirstOrDefault();
                ...
                else if (bNew)
                {
                    ...
                    cml.n = n;
                    cml.core_id = core_id;
                    _db.cmi_comment_from_learner.Add(cml);
                    _db.SaveChanges();
        ...
        public string GetCommentsFromCMIData(int data_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    cmi_data cd = _db.cmi_data.Where(ix => ix.id == data_id).FirstOrDefault();
        ...
        public int GetStudentPreferenceID(int user_id)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var iy = _db.cmi_student_preferences.Where(ix => ix.user_id == user_id).FirstOrDefault();
                if (iy != null)
                {
                    id = iy.id;
                }
                ...
                else
                {
                    ...
                    cmp.user_id = user_id;
                    _db.cmi_student_preferences.Add(cmp);
                    _db.SaveChanges();
                    id = cmp.id;
                    return id;
                }
        ...
        public int GetCommentsFromLMSID(int SCORM_Course_id, string SCO_identifier, int n, bool bNew)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var iy = _db.cmi_comment_from_lms.Where(ix => ix.SCORM_Course_id == SCORM_Course_id && ix.SCO_identifier == SCO_identifier && ix.n == n).FirstOrDefault();
                if (iy != null)
                {
                    id = iy.id;
                }
                ...
                else if (bNew)
                {
                    ...
                    if (max_n == n)
                    {
                        ...
                        cml.n = n;
                        _db.cmi_comment_from_lms.Add(cml);
                        _db.SaveChanges();
                        id = cml.id;
                        return id;
                    }
        ...
        public int GetObjectiveID(int core_id, int n)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var iy = _db.cmi_objectives.Where(ix => ix.core_id == core_id).FirstOrDefault();
                if (iy != null)
                {
                    id = iy.id;
                }
        ...
        public int GetInteractionsCorrectResponses(int interactions_id, int n, bool bNew)
        {
            ...
            try
            {
                using (var _db = getContext())
                {
                    var iy = _db.cmi_interactions_correct_responses.Where(ix => ix.interactions_id == interactions_id && ix.n == n).FirstOrDefault();
                if (iy != null)
                {
                    id = iy.id;
                }
                ...
                else if (bNew)
                {
                    ...
                    if (max_n == n)
                    {
                        ...
                        _db.cmi_interactions_correct_responses.Add(ci);
                        _db.SaveChanges();
                        id = ci.id;
                        return id;
                    }
        ...
        public void SetValueInteractionsCorrectResponses(int id, string sValue)
        {
            try
            {
                using (var _db = getContext())
                {
                    cmi_interactions_correct_responses cicr = _db.cmi_interactions_correct_responses.Where(ix => ix.id == id).FirstOrDefault();
                    cicr.pattern = sValue;
                    _db.SaveChanges();
                }
            }
        ...
        public void SetValueInteractionsObjections(int interaction_n, int n, string objective_id, int interactions_id)
        {
            try
            {
                ...
                _db.cmi_interactions_objectives.Add(cio);
                _db.SaveChanges();
                    }
            }
            catch (Exception ex)
